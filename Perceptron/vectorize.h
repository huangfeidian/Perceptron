...
using std::vector;
#ifdef USE_AVX
float  avx_product(const vector<double>& origin1,const vector<double>& origin2)//2 vector product
{
	int size1 = origin1.size();
	int size2 = origin2.size();
	assert(size1 == size2);
	int blocks = size1 / 4;//because 256bit =8*sizeof(float)
	__m256 result256,temp1,temp2;
	float result=0;
	result256=_mm256_setzero_ps();
	for (int i = 0; i < blocks; i++)
	{
		temp1 = _mm256_loadu_ps(&(origin1[0])+i*4);
		temp2 = _mm256_loadu_ps(&(origin2[0]) + i * 4);
...
float avx_product(const vector<double>& origin1, const vector<double>& origin2, const vector<double>& origin3)//3 vector product
{
	int size1 = origin1.size();
	int size2 = origin2.size();
	...
	for (int i = 0; i < blocks; i++)//maybe we can align these vector or just replace these vector with struct 
	{
		temp1 = _mm256_loadu_ps(&(origin1[0]) + i * 4);
		temp2 = _mm256_loadu_ps(&(origin2[0]) + i * 4);
		temp3 = _mm256_loadu_ps(&(origin3[0]) + i * 4);
...
float avx_product(const vector<double>& origin1, const vector<double>& origin2, const vector<double>& origin3, const vector<double>& origin4)//4 vector product
{
	int size1 = origin1.size();
	int size2 = origin2.size();
	int size3 = origin3.size();
	...
	for (int i = 0; i < blocks; i++)//maybe we can align these vector or just replace these vector with struct 
	{
		...
		temp2 = _mm256_loadu_ps(&(origin2[0]) + i * 4);
		temp3 = _mm256_loadu_ps(&(origin3[0]) + i * 4);
		temp4 = _mm256_loadu_ps(&(origin4[0]) + i * 4);
...
#else
float  avx_product(const vector<double>& origin1,const vector<double>& origin2)
{
	float result=0;
	int size1 = origin1.size();
	int size2 = origin2.size();
